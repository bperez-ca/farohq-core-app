...
func (r *TenantRepository) FindByID(ctx context.Context, id uuid.UUID) (*model.Tenant, error) {
	query := `
		SELECT id, name, slug, status, tier, agency_seat_limit, created_at, updated_at, deleted_at
		FROM agencies
	...
	return r.mapToDomainTenant(dbID, name, slug, status, tier, agencySeatLimit, createdAt, updatedAt, deletedAt), nil
}
...
func (r *TenantRepository) FindBySlug(ctx context.Context, slug string) (*model.Tenant, error) {
	...
	return r.mapToDomainTenant(id, name, dbSlug, status, tier, agencySeatLimit, createdAt, updatedAt, deletedAt), nil
}
...
func (r *TenantRepository) Save(ctx context.Context, tenant *model.Tenant) error {
	...
	query := `
		INSERT INTO agencies (id, name, slug, status, tier, agency_seat_limit, created_at, updated_at, deleted_at)
...
func (r *TenantRepository) mapToDomainTenant(id uuid.UUID, name, slug, status string, tier *string, agencySeatLimit int, createdAt, updatedAt time.Time, deletedAt *time.Time) *model.Tenant {
	...
	return model.NewTenantWithID(id, name, slug, tenantStatus, domainTier, agencySeatLimit, createdAt, updatedAt, deletedAt)
}
